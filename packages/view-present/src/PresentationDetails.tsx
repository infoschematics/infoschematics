  ...
  return (
    <aside className="isp-details" aria-label="Presentation details">
      ...
      {tab === 'info' ? (
        ...
        <div className="isp-details-body">
          <p className="isp-muted">What this Infoschematic conforms to, grouped by the owning specification set.</p>
          {runtime.infoschematicSpecificationSections.map(({ group, within }) => (
            <section className="isp-specification-group" key={group.id}>
              <h2>{group.label}</h2>
              <ul>
                {within.map((entry) => (
                  <li key={entry.id}>
                    {entry.href ? <a href={entry.href}>{entry.label}</a> : <span>{entry.label}</span>}
