...
export type ProductionState =
  | PresentProductionState
  | DesignProductionState
  | DirectProductionState;
...
const withoutPresentationFocus = (
  presentation: PresentationState,
): PresentationState => reducePresentation(presentation, { type: "clear-focus" });
...
export const directTargetIsValid = (target: DirectTarget): boolean => {
  switch (target.kind) {
    ...
    case "callout":
      return (
        hasText(target.ownerId) &&
        hasText(target.sceneId) &&
        (target.owner === "story" || target.owner === "theme")
      );
  }
};
...
export const reduceProduction = (
  ...
  switch (action.type) {
    ...
    case "set-direct-target": {
      ...
      const directTarget =
        action.target && directTargetIsValid(action.target)
          ? action.target
          : null;
      return directTarget === state.directTarget
        ? state
        : { ...state, directTarget };
    }
