...
export const usePresentation = (runtime: InfoschematicRuntime, signalPolicy: SceneSignalPolicy) => {
  ...
  const derived = useMemo(() => derivePresentation(runtime, state, signalPolicy), [runtime, signalPolicy, state])

  useEffect(() => {
    if (!storage) return
    try {
      window.localStorage.setItem(`${storage}.annotated`, String(state.annotated))
      window.localStorage.setItem(`${storage}.presentation.autoAdvance`, String(state.autoAdvance))
      window.localStorage.setItem(`${storage}.takeaways`, String(state.takeaways))
    } catch {
