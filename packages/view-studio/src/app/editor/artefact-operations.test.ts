...
describe('typed artefact operation lifecycle', () => {
  it('retains negative Flow geometry capabilities for command gating', () => {
    ...
    expect(
      moveArtefactOperation(flow, { points: config.infoschematic.flows[0]!.points, role: 'route' }, { dx: 1, dy: 1 })
    ).toBeUndefined()
    expect(
      resizeArtefactOperation(
        flow,
  ...
  it('replaces partial properties for all five kinds without losing authored fields', () => {
    ...
    const operations = replacements.filter((operation): operation is NonNullable<typeof operation> =>
      Boolean(operation)
    ...
    expect(artefactSourceChanges(operations).map((change) => change.target.kind)).toEqual([
      'region',
      'fabric',
      'card',
      'graphic',
  ...
  it('coalesces property edits and keeps them undoable, discardable and ordered', () => {
    ...
    expect(artefactSourceChanges(operations)[0]?.source).toContain('replace card')
    expect(discardArtefactOperation(operations, artefactOperationKey(latest))).toEqual([])

    const changed = {
      ...emptyEditorDraft(),
      artefactOperations: operations as ReturnType<typeof emptyEditorDraft>['artefactOperations']
    }
    const history = applyDiscreteDraft(createEditorDraftHistory(emptyEditorDraft()), changed)
  ...
  it('materialises the latest value for a newly-created target', () => {
    ...
    const replaced = replaceArtefactPropertiesOperation(config, [created], target, {
      kind: 'card',
      value: { detail: 'Edited before rebuild' }
    })!
  ...
  it('coalesces repeated geometry changes and stays serialisable', () => {
    ...
    const operations = recordArtefactOperations(
      [],
      [first, latest, resized].filter(Boolean) as NonNullable<typeof first | typeof latest | typeof resized>[]
    )
  ...
  it('unmakes an artefact created and removed in the same draft', () => {
    ...
    expect(recordArtefactOperation([created], removeArtefactOperation(card))).toEqual([])
  })
  ...
  it('coalesces reorders from the authored index and unmakes a return', () => {
    ...
    expect(recordArtefactOperations([], [first, next])).toEqual([{ ...next, from: 0 }])
  ...
  it('cascades endpoint Flows before owners and removes a Region alone', () => {
    ...
    expect(cardPlan.operations.map((entry) => `${entry.operation}:${entry.target.kind}`)).toEqual([
      'remove:flow',
      'remove:card'
    ])
    expect(fabricPlan.operations.map((entry) => `${entry.operation}:${entry.target.kind}`)).toEqual([
      'remove:flow',
      'remove:fabric'
    ])
  ...
  it('orders source rows deterministically and dependency-safely', () => {
    ...
    expect(changes.map(({ target }) => target.kind)).toEqual(['flow', 'card', 'fabric'])
    expect(changes.map(({ key }) => key)).toEqual(changes.map(({ target }) => `${target.kind}:${target.id}:remove`))
  })
  ...
  it('groups a pointer gesture into one undo step and restores its latest edit', () => {
    ...
    history = applyGestureDraft(history, {
      ...history.current,
      artefactOperations: recordArtefactOperation(history.current.artefactOperations, first)
    })
    history = applyGestureDraft(history, {
      ...history.current,
      artefactOperations: recordArtefactOperation(history.current.artefactOperations, latest)
    })
