...
export function Book({
  ...
  /** Lifted to the app, because the stage marks what the selected scene lights. */
  /** The scene library, lifted for the same reason the stories are. */
  scenes: SceneLibraryEditor
  stories: SceneList
  /** Both need the topology: where there is room on a route, and where its ports are. */
  onAddWaypoint: () => void
  onResetRoute: () => void
  editor: {
    canRedo: boolean
    canUndo: boolean
    canRoute: boolean
    /** Whether the stage selection is a flow, so a scene is given the right list. */
  ...
  const {
    config,
    topologyCardsOffering,
    topologyFlowsCarrying,
    topologySpecificationSections,
    topologyUnroutedInterfaces
  } = useInfoschematic()
  const unroutedInterfaceIds = new Set(topologyUnroutedInterfaces.map((entry) => entry.id))
   ...
   *
   * Front of house and backstage are different work, so returning to one should
  ...
  const [backTab, setBackTab] = useSessionState<'scene' | 'stage' | 'story'>(
    config.id && `${config.id}.panel.tab.backstage`,
    'stage'
  )
  const panelTab = stage.backstage ? backTab : frontTab
  ...
  const chooseTab = (id: 'scene' | 'showing' | 'specifications' | 'stage' | 'story') =>
  ...
  const contractFlows = useMemo(
    () => (selectedContract ? topologyFlowsCarrying(selectedContract.id) : []),
    [selectedContract]
  )
  ...
  const contractCards = useMemo(
    () => (selectedContract ? topologyCardsOffering(selectedContract.id) : []),
    [selectedContract]
  )
  const { runningDemonstration, spotlight, vendor } = stage
   ...
   *
   * Leaving the tab closes both. Entering it returns to the stage editor,
  ...
  const storyChanges = stories.edited
    ...
    : []
  const libraryChanges = scenes.edited ? [{ field: 'points' as const, key: 'spotlights', source: scenes.source }] : []
  const layerChanges = mode === 'story' ? storyChanges : mode === 'scene' ? libraryChanges : []
  const layerSource = mode === 'story' ? stories.source : mode === 'scene' ? scenes.source : editor.source
  const layerDiscard = mode === 'story' ? stories.revert : mode === 'scene' ? scenes.revert : editor.discard
  useEffect(() => {
    // Front of house closes every editor, whatever tab backstage was left on:
    // the mode decides whether an editor is open, and the tab only which.
    const wanted =
      stage.backstage && (panelTab === 'stage' || panelTab === 'scene' || panelTab === 'story') ? panelTab : null
    if (wanted !== mode) setMode(wanted)
  }, [mode, panelTab, setMode, stage.backstage])
  ...
  return (
    <section className="state-panel" aria-label="What is showing">
      ...
      {panelTab === 'stage' || panelTab === 'scene' || panelTab === 'story' ? (
        <div className="editor-tab">
          ...
          <SplitPane>
            {/* One pane, two things in it: what the selection is, and the story
                built on top of the stage. The split's top half is a single
                child, so they share it rather than each claiming a third of the
                tab. */}
            {/* One pane per editor. The stage editor describes a selection; the
                scene editor describes a scene. Neither has any use for the
                other's panel, and rendering both was the whole of why stage
            ...
            <ChangePane
              ...
              count={mode === 'stage' ? editor.changeCount : layerChanges.length}
               ...
               *
               * Both were shown in both, so the pane read the same in stage
               ...
               */
              pending={mode === 'stage' ? editor.pending : layerChanges}
              onRedo={editor.redo}
              onUndo={editor.undo}
              source={layerSource}
            />
        ...
        <div className="contract-body">
          {runningDemonstration ? (
            <p>{runningDemonstration.question}</p>
          ) : vendor ? (
            <>
            ...
            <p className="contract-empty">
              Nothing is lit. Pick anything in the lighting to see what it shows, or run a story to be walked through
              the stage a scene at a time.
            </p>
          )}
        ...
        <div className="contract-body">
          ...
          <p className="register-note specification-lead">
            What the stage conforms to, grouped by whose specification it is and whether there is a document to read.
          ...
          {topologySpecificationSections.map(({ group, within }) => {
            return (
          ...
          {selectedContract ? (
            ...
            <p className="contract-empty specifications-empty">
              Choose a specification to see what it defines, and what on the stage carries or offers it.
            </p>
          )}
        </div>
      )}

      {/* Fixed, so it covers the stage rather than the panel it was opened from. */}
      {reading?.href ? (
