...
| `INFOSCHEMATICS-TOOL-045` | `ready` | `awaiting-review` | Five interaction-layer controls; foreground selection controls; `DESIGN-018`, `DESIGN-019`, amended `DESIGN-015`; `interaction-layer` vocabulary; `elementFromPoint` browser cases; found `TOOL-063` | Close a layer over a stack and edit what is under it; say whether Points need their own layer |
| `INFOSCHEMATICS-TOOL-046` | `ready` | `awaiting-review` | Ordered selection with an anchor; six align edges, two distribute axes, group drag and group nudge; `ADR-INFOSCHEMATICS-027`; `DESIGN-020`, amended `DESIGN-015`; `selection-anchor` vocabulary; found that the Studio stylesheet does not read the Canvas one | Hold a group with Shift, sweep a range, align and distribute it, and undo; decide whether the editor stylesheet should stop shadowing the Canvas one |
| `INFOSCHEMATICS-TOOL-052` | `ready` | `awaiting-review` | TypeScript 7.0.2 and Vite 8.3.0 in two revertible commits; three of four holds gone, `@types/node` rewritten to track `engines.node`; one Vite through `overrides`; found `TOOL-068` | Nothing to look at — confirm the site and Studio still behave, and decide whether `engines.node >= 22` is still what the packages should promise |
