...
_Verify:_ inspect the emphasis tokens in `packages/view-model/src/tokens.ts`, the treatments in `packages/view-canvas/src/styles.css`, the live regions in `packages/view-canvas/src/Canvas.tsx`, and the page-level reduced-motion emulation the browser suites ask for in `scripts/vitest-workspace.ts` against this requirement.
