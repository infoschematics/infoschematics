...
_Evidence:_ `packages/view-model/src/routing.test.ts` covers end movement, bend insertion and orthogonality. `moveRouteEnds` in `packages/view-model/src/routing.ts` is the one construction the draft overlay and the draft document both move a Flow end through, deriving a waypointless route again from the moved port; `joinedToPort` in the same module repairs the run reaching a port of a waypointed route, and `packages/view-model/src/runtime.test.ts` covers a waypointed Flow whose attached component has moved.
...
The rule is not about Cards. Any element the document says carries its code permanently under `APPEAR-018` — a Fabric, an Adapter, a Point, a Region, or a Flow — MUST be treated the same way: one rectangle for the code, resolved once for every renderer and every reason it is being drawn, with the element's own placement answering. An Adapter MUST take the rim below the Card it holds, where its name already is; a Point MUST stack its code clear of its mark and of its label where the label took that side; a Flow MUST take the along-the-route placement its label already resolves to, so the chip the viewer control drags is the chip the document pinned rather than a second one beside it.

_Conformance:_ conforming

_Verify:_ run `bun run --filter=@infoschematics/view-canvas test`, whose editing cases render one document with and without an authored `card.identity` and compare the annotated codes against the chips. Then open a Diagram that authors `card.identity`, zoom until the chips are drawn, and turn the tag control on and off: each Card MUST carry its code exactly once in both states and in the same place in both, while a Fabric that draws no code of its own MUST gain a tag. Pin a code to a Fabric, a Point, a Region, and a Flow, turn the control on, and confirm each is still drawn exactly once and has not moved. Falsified by a code appearing twice, by the code moving as the control is switched, or by an element that draws nothing losing its tag.

_Evidence:_ `resolveCodeBadge` in `packages/view-model/src/code-badge.ts` resolves one placement per element for both renderers and both reasons; `cardText` and `selfCoded` in `packages/view-canvas/src/InfoschematicDiagram.tsx` withhold the annotation for a code already drawn; `packages/view-canvas/src/InfoschematicDiagram.editing.test.tsx` and `packages/view-model/src/code-badge.test.ts` cover the annotated codes with and without an authored chip and the placement each kind takes.
