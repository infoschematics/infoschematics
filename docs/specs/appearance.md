...
### APPEAR-019 — A style is authored and a mode is resolved

An Infoschematic's **style** says what the drawing is, and MUST be authorable: `neutral` is the plain treatment and `blueprint` is the drafting convention. Its **mode** says which ground the drawing is read on, and MUST be resolvable by whoever renders it, because a definition outlives the contexts it is read in. A document MAY state a preferred mode, and `system` MUST mean the author declined to pick rather than naming a third ground. Every palette MUST answer every paint role, and every style MUST be realised on both grounds, so no drawing can be painted half on one ground and half on another.

An interactive drawing MUST let the page resolve the mode, referencing each role as a generated custom property rather than resolving a palette in TypeScript: the reader's preference and the host's override are both the browser's to settle, per [ADR-INFOSCHEMATICS-037](../decisions/ADR-INFOSCHEMATICS-037-a-palette-belongs-to-a-colour-scheme-not-an-outlet.md). The generated projection MUST therefore declare the light ground unconditionally, the dark one behind `prefers-color-scheme`, and each ground behind an attribute for a host that resolved it itself — so the default path needs no script at all, per [ADR-INFOSCHEMATICS-005](../decisions/ADR-INFOSCHEMATICS-005-host-owned-configuration.md). A style MUST land on the drawing and a mode on the document element, so the two compose by specificity rather than by either overruling the other.

The generated projection MUST also restore the light ground under `print` media, declared after both the reader's preference and the host's override so that source order settles it on equal specificity: a dark palette on paper is a page of ink a reader cannot read. An authored `blueprint` MUST print in its own light realisation rather than carrying a dark backdrop onto the page, because paper is a ground and a style is not.

An authored colour MUST be read as a hue seed. Its hue, its saturation and its ordering relative to the author's other colours MUST survive, and the lightness band it is realised in MUST belong to the ground, so a colour chosen against one ground stays legible on the other. An author who meant a literal value MUST be able to say so by ending it with `!`, and a pinned value MUST be drawn exactly as written. A value this rule cannot interpret MUST be passed through untouched rather than guessed at. Readable ink MUST be measured against the fill that was drawn rather than the one that was authored, so a label stays legible on whichever ground it landed on.

_Conformance:_ conforming

_Verify:_ Run `bun run test --filter=@infoschematics/view-model` and the Canvas browser suite, and ask the page for each `prefers-color-scheme` preference and read what the drawing resolved — not what the stylesheet says, because a role one block declares and another omits resolves to whatever was left behind and looks correct in the block that declared it. Confirm a blueprint document is realised on the reader's ground and is still a blueprint on both; confirm an authored colour keeps its hue on both grounds and that a pinned one does not move. Falsified by a palette missing a role, by a literal colour in a Canvas source file, or by a seed whose hue changed. Ask the same page for `print` media: the light ground must come back over a dark preference, in whichever style the drawing is.

_Evidence:_ `packages/view-model/src/tokens.ts` holds one role set per style and ground with `paintVariable` naming each as a custom property; `packages/view-model/src/colour.ts` is the only place an authored colour is interpreted, and `packages/view-model/src/colour.test.ts` holds hue, ordering, translucency and the pin; `scripts/generate-visual-tokens.ts` refuses to write the stylesheet when palettes disagree about roles; `packages/view-canvas/src/Canvas.schemes.browser.test.tsx` proves the resolved ground, the host override and the printed realisation in a browser; `packages/view-canvas/src/tokens.test.tsx` holds the per-block role floor.
...
Canvas MUST provide an accessible name for the complete Infoschematic and for interactive Region, Card, and Flow elements. Hiding optional Card rows visually MUST NOT remove the Card's authored identity, stereotype, or description from its accessible SVG metadata. Domain colour, style, and grid MUST NOT be the only source of meaning.

Representative treatment fixtures MUST prove the same resolved style, grid, frame, label placement, Card detail, compactness, and Domain decisions as static SVG. Tests MAY compare semantic attributes and deterministic geometry rather than byte-identical React and string-rendered markup.

_Conformance:_ conforming

_Verify:_ Run `bun run test --filter=@infoschematics/view-canvas`, then hide the optional Card rows through output options and read the accessible tree rather than the picture: the root description must still summarise visible Card identity, stereotype, and description; each Card's accessible label must still carry its authored values; the whole Infoschematic and every interactive Region, Card, and Flow must still have a name. Read the same document with colour, style, and grid disregarded and confirm nothing was said only by them. Then compare the resolved style, grid, frame, label placement, Card detail, compactness, and Domain decisions against static SVG for the representative fixtures, by semantic attribute and deterministic geometry rather than by markup bytes.
...
_Verify:_ render overlapping filled and framed Regions over neutral and blueprint styles in Canvas and static SVG, then compare layer order and treatment parity.

_Evidence:_ `packages/view-canvas/src/InfoschematicDiagram.treatments.test.tsx`, `packages/render-svg/src/index.test.ts`, and `scripts/visual-treatment-parity.test.ts` cover the current style, grid, Region fill, and frame vocabulary across renderers.
