...
decision_depends_on: [ADR-INFOSCHEMATICS-017, ADR-INFOSCHEMATICS-010]
---

# ADR-INFOSCHEMATICS-022: Rasterise with a native resvg binding

## Context

`ADR-INFOSCHEMATICS-017` made the renderer command a thin Node adapter, and `CLI-005` in [the command-line rendering specification](../specs/command-line-rendering.md) held it to two workspace dependencies and no third-party runtime at all. PNG output is the first capability that cannot be built from the canonical libraries: something has to resolve an SVG document into pixels.
...
The engine implements a subset of SVG, and ignores what it does not implement rather than reporting it. `orient="auto-start-reverse"` and `fill: context-stroke` — both SVG 2, both resolved by every browser the Canvas runs in — are silently dropped, so a marker the interactive renderer expresses once has to be emitted as literal mirrored and literal recoloured definitions for still output. Static output therefore keeps the shared geometry token and diverges on the mechanism, which is stated in both renderers and required by STATIC-013. A feature this engine lacks costs a second definition, not a failed render: nothing warns, and only looking at a PNG finds it.
