...
24. [ADR-INFOSCHEMATICS-018](ADR-INFOSCHEMATICS-018-keep-the-renderer-command-thin.md) — publish document rendering as a thin Node command over canonical libraries.
...
29. [ADR-INFOSCHEMATICS-023](ADR-INFOSCHEMATICS-023-keep-example-packages-copyable-rather-than-published.md) — keep example packages copyable and unpublished rather than members of the coordinated release.
...
41. [ADR-INFOSCHEMATICS-036](ADR-INFOSCHEMATICS-036-an-adapter-card-is-positioned-by-what-it-holds.md) — derive an Adapter Card’s position from the Card it holds in every renderer, so its authored bounds do not position it.
...
42. [GDR-INFOSCHEMATICS-005](GDR-INFOSCHEMATICS-005-name-commands-by-owner-then-subject-then-verb.md) — name every command owner first, then subject, then verb, and hold the surface in one test.
