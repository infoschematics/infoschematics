...
This brief defines the visual character shared by [Infoschematic](/docs/reference/vocabulary/#infoschematic) views and renderers. It is a presentation system rather than a fixed page template: implementations can adapt layout and theme while preserving hierarchy, semantic colour, routing and motion rules.

The [Components guide](/docs/components/) introduces every visible artefact and lets readers compare the authored treatments in rendered SVG. This document explains the principles behind those outputs.
