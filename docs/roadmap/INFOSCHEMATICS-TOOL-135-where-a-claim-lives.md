...
- [ ] Decide where the vocabulary belongs — a guide, a reference document, or folded into `KDR-INFOSCHEMATICS-001` — and record the reasoning in the review rather than only the outcome.
...
`docs/design/*.md`, `docs/guides/*.md`, `docs/reference/*.md`, `apps/site/src/routes.ts`, `apps/site/src/DocumentPage.tsx`, and the vocabulary checks under `scripts/` if the glossary moves. `docs/decisions/KDR-INFOSCHEMATICS-001-product-vocabulary.md` if the vocabulary's home changes what that record governs.
