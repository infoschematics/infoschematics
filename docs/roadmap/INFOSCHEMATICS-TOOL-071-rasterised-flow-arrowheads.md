...
- `packages/view-canvas/src/InfoschematicDiagram.tsx:1950` uses the same attribute in the DOM path. That is correct there and is not in scope, but the two renderers now differ deliberately rather than accidentally, which needs saying in a comment or the difference will be "corrected" later.
...
2. [ ] Correct the stale comment at `packages/render-svg/src/index.ts:342`, and state at `packages/view-canvas/src/InfoschematicDiagram.tsx:1950` why the DOM path keeps the SVG 2 value while the static path does not. Verifiable by reading them.
