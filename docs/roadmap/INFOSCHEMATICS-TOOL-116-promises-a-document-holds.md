...
- [ ] Implement the checks as rules in TOOL-113's checker, so a broken promise is reported with the same code, subject and evidence as a geometric finding.
...
`blocked_by` [INFOSCHEMATICS-TOOL-113](INFOSCHEMATICS-TOOL-113-where-a-drawing-fails.md), which supplies the reporting surface a declared check needs. It blocks nothing.
...
Consumes TOOL-113. Nothing further follows unless declarations turn out to want their own editing surface in Studio, which would be its own record.
...
Adopted for immediate work on 2026-09-21, behind TOOL-113. The data-or-overlay question is taken as part of delivery and recorded as a decision, because it is the one choice here that enlarges what every outlet consumes.
