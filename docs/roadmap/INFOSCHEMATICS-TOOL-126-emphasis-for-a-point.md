...
- [ ] Honour the reduced-motion and accessible-announcement obligations every Dynamic kind already owes under `ADR-INFOSCHEMATICS-024`, asking the runner for the preference through the browser command rather than reading the rule out of the stylesheet.
