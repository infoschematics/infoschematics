...
Infoschematics publishes eight public packages as one coordinated version from one immutable repository tag. Preparing, building, packing, or merging release work does not grant authority to publish. A release owner must deliberately create the tag and start the protected workflow.
...
Choose one SemVer, including a patch for a fix-forward release. Apply the exact version to all eight public package manifests and that same exact version to every dependency between them. Do not independently version one package or include Site or authored examples in the release set.
...
1. Check `npm view <package>@<version>` for all eight names, exact internal dependency ranges, public access, and provenance.
...
npm versions are immutable. Do not move the tag, overwrite a tarball, or unpublish a routine faulty release. Stop the workflow if safe, record any packages published before the failure, deprecate defective versions when consumers need a warning, fix the root cause, and publish a new coordinated patch across all eight packages.
