/**
 * Generated from `overview.yaml` by `bun run self:examples:generate`. Do not edit.
 *
...
diagram:
  ...
  cards:
    ...
    - id: OUT-05
      ...
      bounds: 964 224 240 120
      ports:
        west: 1
  regions:
    - id: inputs
      label: Inputs
      bounds: 24 24 320 360
      appearance:
        label:
  ...
  flows:
    - id: SHAPE
      family: composition
      link: STR-01 E1 -> INFO-03 W1
      labelAt: 0.5
      waypoints: 409,124 409,184
    - id: DIRECT
      family: composition
      link: PRS-02 E1 -> INFO-03 W2
      labelAt: 0.5
      waypoints: 409,284 409,224
    - id: RENDER
      family: output
      link: INFO-03 E1 -> OUT-04 W1
      labelAt: 0.5
      waypoints: 859,184 859,124
    - id: PRESENT
      family: output
      link: INFO-03 E2 -> OUT-05 W1
      labelAt: 0.5
      waypoints: 859,224 859,284
  gridSize: 10
scopes:
  - id: product
    label: Product
...
const parsed = parseInfoschematic(source, { pathname: 'overview.yaml' })
if (!parsed.ok) {
  throw new Error(
    ['overview.yaml is not a valid Infoschematic:', ...parsed.issues.map(formatInfoschematicIssue)].join('\n')
  )
}

/** Structure and presentation combined in one Infoschematic */
export const homepageInfoschematic = parsed.model
